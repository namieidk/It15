...
export function useManagerChat() {
  ...
  const [currentUser, setCurrentUser] = useState<ChatContact | null>(null);
  ...
  useEffect(() => {
    ...
    const user: ChatContact = JSON.parse(storedUser);
    setCurrentUser(user);
    ...
    const init = async () => {
      try {
        ...
        console.error("Manager Directory Load Error:", err);
      } finally {
        setIsReady(true);
      }
    };
