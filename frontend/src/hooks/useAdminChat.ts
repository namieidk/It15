...
export function useAdminChat() {
  ...
  const [currentUser, setCurrentUser] = useState<AdminContact | null>(null);
  const [contacts, setContacts] = useState<AdminContact[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [activeChat, setActiveChat] = useState<AdminContact | null>(null);
  ...
  useEffect(() => {
    ...
    const user: AdminContact = JSON.parse(storedUser);
    setCurrentUser(user);
