...
import { 
  ...
  UserCheck, Loader2, Building2, CheckCircle2, XCircle, Eye, EyeOff, X, Users
} from 'lucide-react';
...
export const ProvisionForm = ({ onBack, onSubmit, formData, setFormData, isSubmitting }: Props) => {
  ...
  // Auto-fill department when name is selected (Optional helper)
  ...
  return (
    <section className="flex-1 flex flex-col overflow-y-auto bg-[#020617] relative font-sans">
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-indigo-500/30 rounded-[2.5rem] p-8">
            <button onClick={handleCloseModal} className="absolute top-6 right-6 text-slate-500 hover:text-white">
              <X className="w-5 h-5" />
            </button>
            <div className="flex flex-col items-center text-center uppercase tracking-widest">
              ...
              <button onClick={handleCloseModal} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-black text-[10px] tracking-widest">ACKNOWLEDGE</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="p-12 border-b border-white/5 flex justify-between items-center">
        <button onClick={onBack} className="flex items-center gap-2 text-indigo-500 font-black text-[10px] tracking-widest uppercase"><ArrowLeft size={16}/> Abort</button>
      ...
      <form onSubmit={internalSubmit} className="p-12 max-w-4xl mx-auto w-full space-y-12 uppercase">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <input required maxLength={6} value={formData.employeeId} onChange={(e) => handleInputChange('employeeId', e.target.value.toUpperCase())} placeholder="AX0000" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-xs font-black text-white outline-none focus:border-indigo-500" />
              </div>

              <div className="space-y-2">
                <label className="text-[8px] font-black text-slate-600 tracking-widest ml-2">Password</label>
                <div className="relative">
                  <input required type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => handleInputChange('password', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 pr-12 text-xs font-black text-white outline-none focus:border-indigo-500" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500">{showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select required value={formData.department} onChange={(e) => handleInputChange('department', e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-2xl p-4 text-xs font-black text-white outline-none focus:border-indigo-500 appearance-none cursor-pointer">
              ...
              <div className="space-y-2">
                ...
                <select required value={formData.role} onChange={(e) => handleInputChange('role', e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-2xl p-4 text-xs font-black text-white outline-none focus:border-indigo-500 appearance-none cursor-pointer">
        ...
        <div className="bg-indigo-600/5 border border-indigo-600/20 p-8 rounded-[2.5rem] flex items-center justify-between">
          ...
          <button disabled={isSubmitting} type="submit" className="px-12 py-5 bg-indigo-600 text-white rounded-2xl font-black text-[10px] tracking-widest hover:bg-indigo-500 transition-all flex items-center gap-2 uppercase">
