...
export default function ManageAccountsPage() {
  ...
  const handleReactivate = async (id: number) => {
    try {
      const response = await fetch(`http://localhost:5076/api/admin/reactivate-account/${id}`, { method: "PUT" });
      if (response.ok) {
  ...
  const executeRevoke = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5076/api/admin/revoke-account/${targetRevokeId}`, { method: "PUT" });
      if (response.ok) {
  ...
  return (
    <SessionGuard allowedRoles={['ADMIN']}>
      ...
      <main className="h-screen w-full flex bg-[#020617] text-slate-200 overflow-hidden font-sans uppercase italic font-black relative">
            ...
            <div className="relative w-full max-w-md bg-slate-900 border border-red-500/30 rounded-[2.5rem] p-10">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="flex flex-col w-full gap-3">
                  <button onClick={executeRevoke} disabled={isSubmitting} className="w-full py-5 bg-red-600 hover:bg-red-500 text-white rounded-2xl font-black text-[10px] tracking-widest transition-all italic">
                    {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : 'CONFIRM TERMINATION'}
                  </button>
                  <button onClick={() => setShowRevokeModal(false)} className="w-full py-5 bg-white/5 text-slate-400 rounded-2xl font-black text-[10px] tracking-widest italic">ABORT</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {view === 'create' && <ProvisionForm onBack={() => setView('list')} onSubmit={handleProvision} formData={formData} setFormData={setFormData} isSubmitting={isSubmitting} />}
        {view === 'edit' && <EditAccountForm onBack={() => setView('list')} onSubmit={handleUpdate} formData={formData} setFormData={setFormData} isSubmitting={isSubmitting} />}
      </main>
    </SessionGuard>
  );
}