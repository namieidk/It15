        ...
        <div className="flex-1 flex flex-col bg-[#020617]">
          {activeChat && currentUser ? (
                  ...
                  .map((msg, i) => {
                    ...
                    return (
                      <div key={i} className={`flex flex-col ${isMe ? 'items-end ml-auto' : 'items-start'} max-w-[70%]`}>
                        {/* Condition: Render raw image OR text bubble */}
                        {msg.fileUrl ? (
                          // Raw image presentation (No bubble)
                          <img 
                            src={msg.fileUrl} 
                            alt="attachment" 
              ...
              <div className="p-10 pt-0">
                ...
                <div className="bg-slate-900/50 border border-white/10 rounded-[2.5rem] p-2 flex items-center gap-2 focus-within:border-indigo-500/50 transition-all shadow-inner backdrop-blur-xl">
                  ...
                  <input 
                    ...
                    placeholder={isUploading ? "UPLOADING SYSTEM ASSETS..." : "TYPE ADMINISTRATIVE COMMAND..."} 
                    className="flex-1 bg-transparent outline-none text-[10px] font-black tracking-widest text-white px-4"
                    disabled={isUploading}
                  />
                  <button 
                    onClick={() => handleSend()} 
                    disabled={(!input.trim() && !isUploading) || isUploading} 
                    className="bg-indigo-600 p-4 rounded-full text-white hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50"
                  >
            ...
            <div className="flex-1 flex flex-col items-center justify-center opacity-20 uppercase tracking-[0.5em]">
              ...
              <p className="text-[10px] font-black">ADMIN COMMAND STANDBY</p>
            </div>
          )}
        </div>
      </section>
