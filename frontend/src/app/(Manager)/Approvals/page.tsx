...
export default function ApprovalsPage() {
  ...
  const handleAction = async (requestId: number, actionType: 'APPROVED' | 'REJECTED') => {
    try {
      // Uses manager-action endpoint — approved goes to HR, rejected stops here
  ...
  return (
    <main className="h-screen w-full flex bg-[#020617] text-slate-200 overflow-hidden font-sans uppercase relative">
      ...
      {!isLoading && fetchError && (
        <div className="flex-1 flex items-center justify-center bg-[#020617]">
          <div className="flex flex-col items-center gap-6 max-w-md text-center">
            ...
            <div>
              <p className="text-[10px] font-black tracking-[0.3em] text-red-400 mb-2">
                CONNECTION FAILED
              </p>
              <p className="text-[11px] text-slate-500 font-bold tracking-widest leading-relaxed">
                {fetchError}
              </p>
            </div>
            <button
      ...
      {/* Main Content */}
      {!isLoading && !fetchError && (
        <ManagerApprovalsUI requests={requests} onAction={handleAction} />
      )}
    </main>
  );
}