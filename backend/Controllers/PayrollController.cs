...
namespace YourProject.Controllers
{
    ...
    public class PayrollController : ControllerBase
    {
            ...
            try
            {
                var users = await _context.Users
                    ...
                    .Select(u => new
                    {
                        ...
                        HasProfile = _context.EmployeeProfiles
                                        .Any(p => p.EmployeeId == u.EmployeeId)
                    })
                    .ToListAsync();
                ...
                var result = users.Select(u =>
                {
                    var profile = _context.EmployeeProfiles
                        .FirstOrDefault(p => p.EmployeeId == u.id);

                    var salary     = profile?.BasicMonthlySalary ?? 0;
                    var deductions = CalculateDeductions(salary);
                    var grossPay   = salary; // No OT/allowances in roster preview
        ...
        // ──────────────────────────────────────────────────────────────────────
        // 5. BATCH PROCESS
        //    - Pulls attendance for the period to compute OT & Night Diff
        //    - Computes all deductions + withholding tax
        //    - Saves Payslip records
        // ──────────────────────────────────────────────────────────────────────
        ...
        public async Task<IActionResult> BatchProcess([FromBody] BatchProcessRequest req)
        {
            ...
            foreach (var empId in req.EmployeeIds)
            {
                try
                {
                    ...
                    // ── Pull attendance records for this period ──────────────
                    ...
                    // ── Compute OT & Night Diff from attendance ──────────────
                    ...
                    decimal allowances   = 0m; // Extend here if you have an allowance table
                    ...
                    decimal  totalDeductions = Math.Round(
                                                deductions.Total + withholdingTax, 2);
        ...
        // ──────────────────────────────────────────────────────────────────────
        // 6. GET PAYSLIPS  (all or by employee)
        // ──────────────────────────────────────────────────────────────────────
        [HttpGet("payslips")]
        ...
        public async Task<IActionResult> GetPayslip(int id)
        {
            try
            {
                ...
                if (p == null)
                    return NotFound(new { message = "PAYSLIP_NOT_FOUND" });

                var user = await _context.Users
                    .FirstOrDefaultAsync(u => u.EmployeeId == p.EmployeeId);
        ...
        // 2026 PH Rates — Employee Share Only
        ...
        // 2026 PH Withholding Tax — Monthly Gross
        private static decimal CalculateWithholdingTax(decimal monthlyGross)
        {
            ...
            if (monthlyGross <= 33332m)
                return Math.Round((monthlyGross - 20833m) * 0.15m, 2);
            if (monthlyGross <= 66666m)
                return Math.Round(1875m + (monthlyGross - 33333m) * 0.20m, 2);
            if (monthlyGross <= 166666m)
                return Math.Round(8541.80m + (monthlyGross - 66667m) * 0.25m, 2);
            if (monthlyGross <= 666666m)
                return Math.Round(33541.80m + (monthlyGross - 166667m) * 0.30m, 2);

            return Math.Round(183541.80m + (monthlyGross - 666667m) * 0.35m, 2);
        }

        // Night Differential: 10% extra for hours worked between 10 PM – 6 AM
        private static double ComputeNightDiffHours(DateTime? clockIn, DateTime? clockOut)
        {
            if (clockIn == null || clockOut == null) return 0;

            var nightStart = TimeSpan.FromHours(22); // 10:00 PM
            var nightEnd   = TimeSpan.FromHours(6);  // 06:00 AM
            ...
            while (current < clockOut.Value)
            {
                ...
                bool isNight = t >= nightStart || t < nightEnd;
                if (isNight) nightHours += (next - current).TotalHours;
